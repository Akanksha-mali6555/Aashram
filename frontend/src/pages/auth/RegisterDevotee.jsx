...
const RegisterDevotee = () => {
  ...
  const handleSearchRelatives = async () => {
    if (!searchQuery.trim() && !searchState.trim() && !searchVillage.trim())
      return;
    setLoading(true);
    setError("");
    try {
      let queryParams = [];
      if (searchQuery.trim())
        queryParams.push(`q=${encodeURIComponent(searchQuery.trim())}`);
      if (searchState.trim())
        queryParams.push(`state=${encodeURIComponent(searchState.trim())}`);
      if (searchVillage.trim())
        queryParams.push(`village=${encodeURIComponent(searchVillage.trim())}`);
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    if (formData.registerOption === "joinFamily" && !formData.relativeId) {
      return setError(
        "Please search and select a relative family member to join.",
      );
    }
        ...
        <div className="w-full max-w-xl mx-auto">
          {step > 1 && step < 3 && (
            <button
              ...
              className="text-slate-500 hover:text-orange-600 flex items-center gap-2 mb-6 font-bold transition-colors uppercase tracking-wider text-xs"
            >
          ...
          <div className="bg-white/90 backdrop-blur-2xl border border-white shadow-[0_20px_50px_-15px_rgba(0,0,0,0.05),0_0_30px_rgba(255,165,0,0.05)] p-6 sm:p-8 md:p-12 rounded-[2.5rem] relative overflow-hidden group">
            ...
            {step === 1 && (
              ...
              >
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50/50 backdrop-blur-sm border border-slate-200 text-slate-800 rounded-2xl pl-11 pr-4 py-4 outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-400/10 focus:bg-white transition-all placeholder:text-slate-400 font-bold shadow-sm"
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50/50 border border-slate-200 text-slate-800 rounded-2xl pl-11 pr-4 py-3.5 outline-none focus:border-orange-400 focus:bg-white text-xs font-bold transition-all shadow-sm"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-slate-700 text-xs font-black mb-1.5 uppercase tracking-wider ml-1">
                      Password
                    </label>
                    <div className="relative">
                      <Lock
                      ...
                      <button
                        ...
                        className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-orange-500 transition-colors z-10 p-1"
                      >
                        {showPassword ? (
                          <EyeOff size={18} />
                        ) : (
            ...
            {step === 2 && (
              <form
                ...
                className="space-y-4 relative z-10 max-h-[60vh] overflow-y-auto pr-2"
              >
                ...
                <div className="grid grid-cols-2 gap-4 pb-2">
                  <button
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, registerOption: "newFamily" })
                    }
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, registerOption: "joinFamily" })
                    }
                ...
                {formData.registerOption === "joinFamily" ? (
                  <div className="space-y-4 bg-orange-50/20 p-4 border border-orange-100 rounded-2xl">
                    <label className="block text-slate-700 text-[10px] font-black uppercase tracking-wider">
                      Search and link to relative family member:
                    </label>
                    ...
                    {selectedRelative && (
                      <div className="p-3 bg-emerald-50 border border-emerald-100 text-emerald-800 rounded-xl flex items-center justify-between text-xs font-black">
                        <span>
                          Selected Relative: {selectedRelative.name} (
                          {selectedRelative.devoteeId})
                        </span>
                    ...
                    {selectedRelative && (
                      <div>
                        <label className="block text-slate-700 text-[10px] font-black mb-1.5 uppercase tracking-wider ml-1">
                          Relationship to {selectedRelative.name}
                        </label>
                        <select
                  ...
                  <div className="p-3 bg-orange-500/10 border border-orange-200 text-orange-800 rounded-xl text-xs font-bold">
                    You will be registered as a standalone family head. You can
                    add more parents, kids, or spouses once your profile is
                    active.
                  </div>
                )}
            ...
            {step === 3 && (
              <form
                ...
                className="space-y-6 relative z-10 animate-[fadeIn_0.4s_ease-out]"
              >
                <div>
                  <label className="block text-slate-700 text-sm font-black mb-3 text-center uppercase tracking-widest">
                    Enter 6-Digit OTP
                  </label>
                  <input
                    required
                    ...
                    className="w-full bg-slate-50/50 border border-slate-200 text-slate-800 rounded-2xl px-4 py-5 text-center text-3xl tracking-[0.75em] font-black outline-none focus:border-orange-400 focus:bg-white transition-all placeholder:text-slate-300 shadow-sm"
                    placeholder="------"
                  />
                </div>
